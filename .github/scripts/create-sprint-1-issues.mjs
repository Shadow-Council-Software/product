...
const REPO = 'Shadow-Council-Software/product';
const EPIC = 1;
const MILESTONE = 'Sprint 1 — Runtime Foundation';
...
function body(taskId, acceptance, files, wct, gate, component) {
  return `## OpenSpec task ID
\`${taskId}\`
...
## OpenSpec reference
\`enterprise/openspec/changes/sprint-1-runtime-foundation/tasks.md\` — section ${taskId}
...
const created = [];
for (const [taskId, title, labels, component, wct, gate, acceptance, files] of TASKS) {
  const b = body(taskId, acceptance, files, wct, gate, component);
  const bodyFile = join(tmpdir(), `s1-issue-${taskId}.md`);
  writeFileSync(bodyFile, b);
  const args = [
    'issue', 'create', '--repo', REPO, '--title', title, '--body-file', bodyFile,
    '-m', MILESTONE, '-a', '@me',
    ...labels.split(',').flatMap((l) => ['-l', l]),
  ];
  const url = execFileSync('gh', args, { encoding: 'utf8' }).trim();
  unlinkSync(bodyFile);
  const num = Number(url.split('/').pop());
  created.push({ taskId, num });
  console.log(`Created #${num} — ${taskId}`);
}
